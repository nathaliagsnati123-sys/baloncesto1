...
function OptimizedImage({
  ...
  const hasBgInWrapper = wrapperClassName.includes('bg-');

  return (
    <div className={`relative overflow-hidden rounded-xl ${hasBgInWrapper ? wrapperClassName : `bg-slate-100/50 ${wrapperClassName}`}`}>
...
export default function App() {
  ...
  const [checkoutUrl, setCheckoutUrl] = useState<string>("https://pay.hotmart.com/O107175812D?off=dz8snp6h&checkoutMode=10");
  ...
  useEffect(() => {
    ...
    const baseParams = new URLSearchParams("off=dz8snp6h&checkoutMode=10");
    ...
    <div className="min-h-screen bg-white text-blue-950 font-sans selection:bg-orange-500 selection:text-white overflow-x-clip">
      ...
      <section id="muestra" className="pt-12 pb-6 md:pt-16 md:pb-8 px-4 bg-white border-y border-slate-100 relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          ...
          <div 
            className="mb-4 overflow-hidden relative rounded-3xl border border-slate-200/60 bg-white p-4 py-6 shadow-xl backdrop-blur-md"
            ...
            >
              ...
              <div className="flex gap-5">
                {SCROLL_IMAGES.map((imgUrl, i) => (
                  <div 
                    key={`scroll-img-1-${i}`}
                    className="relative h-[340px] sm:h-[460px] md:h-[520px] flex-shrink-0 bg-white border border-slate-200/80 hover:border-orange-500/50 rounded-2xl overflow-hidden shadow-lg hover:shadow-[0_0_25px_rgba(249,115,22,0.15)] transition-all duration-300 group flex items-center justify-center"
                  >
                    <OptimizedImage 
                      src={imgUrl} 
                      ...
                      loading="lazy"
                      wrapperClassName="h-full w-auto bg-white flex items-center justify-center"
                    />
                  </div>
                ))}
              </div>
          ...
          <div className="bg-white border-[3px] border-orange-500 rounded-[32px] shadow-2xl p-4 md:p-8 space-y-5 relative overflow-visible">
            ...
            <div className="max-w-xl mx-auto space-y-3 text-left text-xs sm:text-sm font-semibold text-slate-800 py-4">
              ...
              {BONUSES.map((bonus) => (
                <div key={bonus.id} className="flex items-start gap-3">
                  <Check className="w-4.5 h-4.5 text-orange-500 mt-0.5 flex-shrink-0 stroke-[3]" />
                  <span>Bono {bonus.number}: {bonus.title} <span className="text-orange-500/90 font-medium">(Valorizado en {convertAndFormat(bonus.originalPrice)})</span></span>
                </div>
              ))}
              <div className="flex items-start gap-3">
                <Check className="w-4.5 h-4.5 text-orange-500 mt-0.5 flex-shrink-0 stroke-[3]" />
            ...
            <div className="space-y-1.5 py-4 border-t border-slate-100/80">
              ...
              <div className="flex items-baseline justify-center gap-1">
                <span className="text-7xl sm:text-8xl md:text-9xl lg:text-[7rem] font-black text-orange-500 font-sans tracking-tight leading-none">
                  {formattedPrice}
                </span>
              </div>
            </div>
